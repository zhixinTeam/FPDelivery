...
uses
  ...
  cxButtonEdit, dxSkinsdxLCPainter;
...
begin
  ...
  try
    ...
    nStr := 'UPDate #qichu Set C_Init=C_Init- xMoney From ( ' +
            'Select R_CusID, -1*IsNull(Sum(IsNull(R_Value,0)), 0) xMoney From %s ' +
            'Where R_Date<''$ST''  ' +
            'Group  by R_CusID ) a  Where R_CusID=C_ID ' ;
    nStr := Format(nStr, [sTable_BackRecord]);
    nList.Add(nStr);
    //合并返还
    ...
    nStr := 'UPDate #qichu Set C_FLMoney=xMoney From ( ' +
            'Select R_CusID, Sum(ISNULL(R_Value,0))*(-1) xMoney From %s ' +
            'Where R_Date>=''$ST'' And R_Date<''$ED'' ' +
            'Group  by R_CusID ) a  Where R_CusID=C_ID ';
    nStr := Format(nStr, [sTable_BackRecord]);
    nList.Add(nStr);
    //结算返利
    ...
    nStr := 'Select * From #qichu Where 1=1 '+FWhere+' Order by c_id ';
...
procedure TfFrameCusTotalMoney.EditCusPropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
begin
  if Trim(EditCus.Text)<>'' then
    FWhere:= ' And C_CusName like ''%%'+Trim(EditCus.Text)+'%%'' '
  else FWhere:= '';

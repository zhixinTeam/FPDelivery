...
begin
  ...
  begin
    ...
    //当前 + 时长
    EditPayment.ItemIndex := EditPayment.Properties.Items.IndexOf(FieldByName('C_Payment').AsString);
...
begin
  ...
  try
    ...
    begin
      ...
      nList.Add(Format('Z_Freight=%s', ['0']));
    end
    else
    begin
      nList.Add(Format('Z_FPType=%s', [IntToStr(EditFPType.ItemIndex+1)]));
      nList.Add(Format('Z_Freight=%s', [FloatToStr(EditFreight.value)]));
    end;

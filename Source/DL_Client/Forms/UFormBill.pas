...
type
  TfFormBill = class(TfFormNormal)
    ...
    EditCarModel: TcxTextEdit;
    dxLayout1Item15: TdxLayoutItem;
    EditXZValue: TcxTextEdit;
  ...
  public
    ...
    function GetLoadStand(TruckNo:string):Boolean;
  end;
...
begin
  {$IFDEF FPST}
  if not GetLoadStand(Trim(EditTruck.Text)) then
  begin
    ShowMsg('车辆信息不存在,请先维护车辆信息.',sHint);
    EditTruck.SetFocus;
    exit;
  end;
  if StrToFloat(EditMaxValue.text) < StrToFloat(EditValue.text) then
  begin
    ShowMsg('超出最大开单量',sHint);
    EditValue.SetFocus;
    exit;
  end;
  {$ENDIF}
...
begin
  ...
  nRet := true;
  if (FBuDanFlag <> sFlag_Yes) and (editCard.Text <> '') then
    nRet := SaveBillCard(gInfo.FIDList, EditCard.Text)
  else
...
begin
  inherited;
  {$IFDEF FPST}
  GetLoadStand(Trim(EditTruck.Text));
  {$ENDIF}
end;

function TfFormBill.GetLoadStand(TruckNo:string):Boolean;
var
  nStr: string;
begin
  Result := False;
  ...
  begin
    ...
    EditMaxValue.Text :=  FieldByName('MaxValue').AsString;
    editCard.Text     :=  FieldByName('T_Card').AsString;
    Result := True;
  end;
end;

...
procedure Create_SrvWebchat(out anInstance : IUnknown);
begin
  anInstance := TSrvWebchat.Create;
end;
...
begin
  ...
  begin
    ...
    FSrvWebchat := TROPooledClassFactory.Create('SrvWebchat',
                Create_SrvWebchat, TSrvWebchat_Invoker,
                FPoolSizeWebChat, TRoPoolBehavior(FPoolBehaviorWebChat));
  end;
end;
...
begin
  ...
  if Assigned(FSrvWebchat) then
  begin
    UnRegisterClassFactory(FSrvWebchat);
    FSrvWebchat := nil;
  end;
end;

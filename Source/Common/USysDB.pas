...
const
  ...
  sFlag_NoBatchAuto   = 'NoBatchAuto';               //品种无需批次号
  ...
  sTable_LoadStandard = 'Sys_LoadStandard';          //荷载标准
  ...
  sSQL_NewFactorys = 'Create Table $Table(R_ID $Inc, F_ID varChar(32),' +
       ...
       'F_WechatUrl varChar(128), F_DBConn varChar(500), F_Area varChar(16),' +
       'F_Valid Char(1), F_Type varChar(32), F_IsMB Char(1), F_Index Integer)';
  {-----------------------------------------------------------------------------
   工厂列表: Factorys
   *.R_ID: 编号
   *.F_ID: 工厂编号
  ...
  sSQL_NewAuditTruck = 'Create Table $Table(R_ID $Inc, A_ID varChar(50),' +
       ...
       'A_WeiXin varChar(20), A_Phone varChar(20), A_LicensePath varChar(200), ' +
  ...
  sSQL_NewLoadStandard = 'Create Table $Table(R_ID $Inc,S_No varchar(20),'+
       'S_CarModel varChar(50),' +
       'S_Value $Float, S_Man varChar(20), S_Memo varChar(15), S_Date DateTime)';
  {-----------------------------------------------------------------------------
   荷载标准表:LoadStandard
   *.R_ID:记录编号
   *.S_CarModel:车型
...
//------------------------------------------------------------------------------
  ...
  -----------------------------------------------------------------------------}
  sSQL_NewAdjustMoney = 'Create Table $Table(R_ID $Inc, A_OutCusId varchar(20),'
      +'A_OutCusName varchar(80), A_InCusId varchar(20), A_InCusName varchar(80),'
      +'A_Value $Float, A_User varchar(30),A_Date DateTime, '
      +'A_Memo varchar(250))';
  {-----------------------------------------------------------------------------
   账户金额调拨表: AdjustMoney
   *.R_ID: 记录编号
   *.A_OutCusId, A_OutCusName: 商城订单
   *.A_InCusId, A_InCusName: 提货单
   *.A_Value: 调拨金额
   *.A_User: 操作员
   *.A_Date: 日期
   *.A_Memo: 备注
  -----------------------------------------------------------------------------}
  sSQL_NewBackCashRule = 'Create Table $Table(R_ID $Inc, B_ZhiKa varchar(30),'
      +'B_StockNo varchar(30),B_LeaveL $Float,B_LeaveH $Float, B_Value $Float, B_Date Datetime,'
      +'B_User varchar(30))';
  {-----------------------------------------------------------------------------
   返现表: BackCashRule
   *.R_ID: 记录编号
...
begin
  ...
  AddSysTableItem(sTable_LoadStandard, sSQL_NewLoadStandard);
